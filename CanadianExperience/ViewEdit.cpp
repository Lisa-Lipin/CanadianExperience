...
BEGIN_MESSAGE_MAP(CViewEdit, CScrollView)
    ...
    ON_WM_MOUSEMOVE()
    ON_WM_LBUTTONDBLCLK()
    ON_COMMAND(ID_EDIT_CHANGESPEECH, &CViewEdit::OnEditChangespeech)
END_MESSAGE_MAP()
...
void CViewEdit::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    picture->ChooseMachine(point.x, point.y);

    picture->UpdateObservers();
}


void CViewEdit::OnEditChangespeech()
{
    auto picture = GetPicture();
    CSpeechDlg dlg;
    dlg.SetActor(picture->ChangeSpeech().get());
    if (dlg.DoModal() == IDOK)
    {
        dlg.Take();
    }
