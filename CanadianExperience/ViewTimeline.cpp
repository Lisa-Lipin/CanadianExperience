...
BEGIN_MESSAGE_MAP(CViewTimeline, CScrollView)
    ...
    ON_WM_TIMER()
//    ON_WM_MBUTTONDBLCLK()
    ON_COMMAND(ID_EDIT_CHANGEMACHINESTARTTIMES, &CViewTimeline::OnEditChangemachinestarttimes)
...
void CViewTimeline::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    if (mMovingPointer && nFlags & MK_LBUTTON)
    {
        ...
        GetPicture()->SetMachineFrame(GetPicture()->GetTimeline()->GetCurrentFrame());
        UpdateWindow();
    }
    else
    {
 ...
 */
 ...
 /**create dlg to change machine start times*/
 void CViewTimeline::OnEditChangemachinestarttimes()
 {
     CMachineStartDlg dlg;
     dlg.SetPicture(GetPicture().get());
     if (dlg.DoModal() == IDOK)
     {
         dlg.Take();
     }
